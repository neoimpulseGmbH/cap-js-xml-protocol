...
class XMLAdapter extends HttpAdapter {
  ...
  get router() {
    ...
    router.use((req, res, next) => {
      if (req.method in { POST: 1, PUT: 1, PATCH: 1 } && req.headers['content-type']) {
        ...
        if (!parts[0].match(/^application\/xml$/) || parts[1] === '') {
          throw cds.error('INVALID_CONTENT_TYPE_ONLY_XML', { statusCode: 415, code: '415' }) // FIXME: better i18n + use res.status
        }
      }
        ...
        req.on("end", async () => {
          try {
            const jsonBody = await parseStringPromise(xmlBody, {
              explicitArray: false, // Arrays nur erstellen, wenn nötig
              ignoreAttrs: true, // Entfernt das `$`-Objekt mit Attributen
              tagNameProcessors: [(name) => name.replace(/^.*:/, "")], // Entfernt Namensräume (z. B. `n0:`)
            });
            req.body = jsonBody;
    ...
    // error handling
    if (cds.env.features.rest_error_handler !== false) // REVISIT: unofficial hack for afc!!!
